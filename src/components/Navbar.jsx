...
export default function Navbar() {
  ...
  const toggleDropDownStatus = (setState, iconRef) => {
    setState((state) => {
      if (state) {
        iconRef.current.style.fill = '#000'
        iconRef.current.style.color = '#000'
      } else {
        iconRef.current.style.fill = '#0D71E2'
        iconRef.current.style.color = '#0D71E2'
      }
      ...
      <div className='navbar__right'>
        <div className='navbar__right__info'>
          ...
          <button
            ...
            onClick={() => toggleDropDownStatus(setCreateDropDownOpen, createDropDownRef)}>
            <AddIcon className='navbar__right__icon' ref={createDropDownRef} />
          ...
          <CreateDropDown
            ...
            iconRef={createDropDownRef}
          />

          <button
            className='navbar__right__button navbar__right__messenger__button'
            onClick={() => toggleDropDownStatus(setMessengerDropDownOpen, messengerDropDownRef)}>
            <svg
              viewBox='0 0 28 28'
              alt=''
          ...
          <MessengerDropDown
            ...
            iconRef={messengerDropDownRef}
          />

          <button
            className='navbar__right__button navbar__right__notifications__button'
            onClick={() => toggleDropDownStatus(setNotificationsDropDownOpen, notificationsDropDownRef)}>
            <svg
              viewBox='0 0 28 28'
              alt=''
              className='navbar__right__icon'
          ...
          <NotificationsDropDown
            isDropDownOpen={notificationDropDownOpen}
            setNotificationsDropDownOpen={setNotificationsDropDownOpen}
            outsideClickIgnoreClass={'navbar__right__notifications__button'}
            iconRef={notificationsDropDownRef}
          />

          <button
            className='navbar__right__button navbar__right__account__button'
            onClick={() => toggleDropDownStatus(setAccountDropDownOpen, accountDropDownRef)}>
            <ArrowDropDownRoundedIcon
              className='navbar__right__icon navbar__right__icon__arrow'
